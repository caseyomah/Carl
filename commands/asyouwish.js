...
watchReacts=function(m,f,l,k) {
    const filter = (reaction) => true||reaction.emoji.name === '✨';m.createReactionCollector(filter).on('collect', (r,c) => {
        t=[];
        log[f].forEach((v,i)=>{if (i!==k) t.push(v)});
        log[f]=t;
        let pings="<@"+m.author+">";
        if (m.reactions.has("☝️")) m.reactions.get("☝️").users.forEach(u=>pings+="<@"+u.id+">");
        chatchan.send(pings+l[2]+" is "+(err?"fixed":"up")+".");
...
if (!reacts) {
    reacts=true;
    Object.keys(log).forEach(f=>log[f].forEach((l,k)=>chan.fetchMessage(l[1]).then(m=>watchReacts(m,f,l,k)).catch()));
}
    if (message.channel===chan) {
        ...
        if (deleteMsg) {
            ...
            CSV.writeArraySync(filepath+type[0]+"."+ext,log[type[0]].slice(-1));
            watchReacts(message,type[0],log[type[0]].slice(-1),(log[type[0]].length-1));
        }
    }
}
